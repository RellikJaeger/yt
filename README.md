...
```bash
yes | (termux-setup-storage && apt update && apt upgrade -y && pkg update && pkg install x11-repo && pkg install git python python-pip ffmpeg phantomjs && python -m pip install --upgrade --force-reinstall yt-dlp && git clone -b Termux https://github.com/RellikJaeger/yt && cd yt && chmod a+x yt && mkdir -p $HOME/bin && mv yt $HOME/bin && cd .. && rm -rf yt && echo "yt \$*" > $HOME/bin/termux-url-opener && chmod a+x $HOME/bin/termux-url-opener) && rm -rf $HOME/../usr/bin/yt $HOME/.local/bin/yt && clear && yt -v && yt -h
```
### Usage example
- Copy and paste this into Termux.
```bash
